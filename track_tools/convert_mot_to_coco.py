...
# DATA_PATH = 'C:/Users/danny/Downloads/MOT17/MOT17'
# DATA_PATH = 'C:/Users/ippr00/Downloads/TransTrack/TransTrack/boat_dataset/'
DATA_PATH = 'C:/Users/ippr00/Downloads/TransTrack/TransTrack/show007/'
...
if __name__ == '__main__':
    ...
    for split in SPLITS:#SPLITS = ['train_half', 'val_half', 'train', 'test']
        if split == "test":
        ...
        for seq in sorted(seqs):
            ...
            ann_path = os.path.join(seq_path, 'gt/MOT_gt.txt')
            ...
            for i in range(num_images):
                ...
                image_info = {'file_name': '{}/img1/{:06d}.jpg'.format(seq, i),  # image name.
            ...
            if split != 'test':
                # det_path = os.path.join(seq_path, 'gt/MOT_gt.txt')
                anns = np.loadtxt(ann_path, dtype=np.float32, delimiter=',')
                # dets = np.loadtxt(det_path, dtype=np.float32, delimiter=',')
                if CREATE_SPLITTED_ANN and ('half' in split):
                    anns_out = np.array([anns[i] for i in range(anns.shape[0])
                                         if int(anns[i][0]) >= image_range[0] and
                                         int(anns[i][0])  <= image_range[1]], np.float32) 
                    # print(anns_out.shape)
                    anns_out[:, 0] -= image_range[0]
                    gt_out = os.path.join(seq_path, 'gt/MOT_gt_{}.txt'.format(split))
                    fout = open(gt_out, 'w')
                    for o in anns_out:
                ...
                #     det_out = os.path.join(seq_path, 'gt/MOT_gt_{}.txt'.format(split))
                #     dout = open(det_out, 'w')
                #     for o in dets_out:
                ...
                print('{} ann images'.format(int(anns[:, 0].max()) + 1))
