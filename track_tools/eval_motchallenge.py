...
string.""", formatter_class=argparse.RawTextHelpFormatter)
    ...
    parser.add_argument('--groundtruths', type=str, default='boat_dataset/test', help='Directory containing ground truth files.')
    parser.add_argument('--tests', type=str, default='output/test/tracks', help='Directory containing tracker result files')
    parser.add_argument('--score_threshold', type=float, help='Score threshold',default=-1)
    parser.add_argument('--gt_type', type=str, default='test')
    parser.add_argument('--eval_official', action='store_true', default=True)
...
if __name__ == '__main__':
    ...
    gtfiles = glob.glob(
      os.path.join(args.groundtruths, '*/gt/MOT_gt.txt'))
